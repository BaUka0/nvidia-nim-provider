...
- **Configuration Unit Test Suite (`tests/config.test.ts`, `tests/api.test.ts`, `tests/summarizer.test.ts`, `tests/provider/chat-provider.stream.test.ts`).** Comprehensive tests covering defaults, boundary clamping, legacy fallback mappings, first-token timeout cancellation, dedicated summarization models, and advanced failover triggers.
