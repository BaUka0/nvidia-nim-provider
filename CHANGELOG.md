...
- **Proposed chatProvider UX Surface (`src/models/catalog.ts`, `src/models/discovery.ts`, `src/provider/chat-provider.ts`).** `mapToChatInformation` now populates proposed `LanguageModelChatInformation` fields with runtime-safe fallbacks: `isBYOK: true` for all curated models, `statusIcon` (`cloud` ThemeIcon), and a `warningText.deprecated` markdown banner for catalog entries whose display name carries the `(Deprecated)` marker (currently `moonshotai/kimi-k2.6`). These fields pass through VS Code's provider-info validation ungated and are safe on stable builds.
- **Opt-In Agent Edit Tool Hints (`nvidia-nim.ui.editToolsHint`, `src/models/catalog.ts`).** Added `KNOWN_EDIT_TOOLS` registry (`find-replace`, `multi-find-replace`, `apply-patch`, `code-rewrite`) and `getEditToolsHint(modelId)`: explicit `editTools` overrides in `NvidiaModelCatalogEntry` win (unknown names filtered), tool-calling models default to `["find-replace", "multi-find-replace"]`, text-only models advertise none. Emission of `capabilities.editTools` is disabled by default because VS Code hard-gates that field behind the proposed `chatProvider` API (`checkProposedApiEnabled` in `extHostLanguageModels.ts`) — enabling it on stock installs would abort model listing with `CANNOT use API proposal: chatProvider`. The hint is emitted only when both guards pass: the user enabled `nvidia-nim.ui.editToolsHint` AND the host actually grants the proposal (extension development mode or Insiders, detected via `ExtensionContext.extensionMode` / app name at activation). Toggling the setting fires `onDidChangeLanguageModelChatInformation` to refresh the picker.
...
- **Proposals Declaration (`package.json`).** `enabledApiProposals` now declares `chatProvider` alongside `languageModelThinkingPart` so development-mode runs can exercise the gated surface; installed stable builds ignore the declaration and continue operating through the guarded fallbacks.
