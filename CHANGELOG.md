...
- **Sampling Defaults for Loop-Prone Models (`src/provider/request-builder.ts`).** When no explicit `frequency_penalty`/`presence_penalty` is configured and no `top_p` is set, low-temperature profiles (`temperature<=0.2`) now receive conservative defaults (`frequency_penalty=0.2` + `presence_penalty=0.1`), reducing degenerate repetition risk for DeepSeek/GLM without affecting high-temperature models or leaking presence when frequency is explicitly disabled (`hasExplicitTopP` + `frequencyPenaltyAutoApplied` guards).
