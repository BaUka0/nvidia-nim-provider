...
- **Configuration Unit Test Suite (`tests/config.test.ts`, `tests/api.test.ts`, `tests/summarizer.test.ts`, `tests/provider/chat-provider.stream.test.ts`, `tests/provider/request-builder.test.ts`, `tests/tools-parser.test.ts`).** Comprehensive tests covering defaults, boundary clamping, legacy fallback mappings, first-token timeout cancellation, dedicated summarization models, advanced failover triggers, generation hyperparameters, and tool execution flags.
