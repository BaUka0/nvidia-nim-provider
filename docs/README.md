...
| `nvidia-nim.generation.frequencyPenalty` | `number \| null` | `null` | `-2` .. `2` | **Frequency penalty.** Positive values discourage verbatim token repetition. `null` disables. Auto `0.2` for low-temp (`DeepSeek 0`/`GLM 0.1`) when no penalty/topP, and `0.15` for Nemotron (`temp 1/topP 0.95`) via adapter default — both without user config. |
