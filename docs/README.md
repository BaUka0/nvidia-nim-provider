...
| `nvidia-nim.generation.frequencyPenalty` | `number \| null` | `null` | `-2` .. `2` | **Frequency penalty.** Positive values discourage verbatim token repetition. `null` omits the parameter from the request (avoids HTTP 400 on NIM models where penalties are immutable). |
| `nvidia-nim.generation.presencePenalty` | `number \| null` | `null` | `-2` .. `2` | **Presence penalty.** Positive values discourage repeating the same topics. `null` omits the parameter from the request (avoids HTTP 400 on NIM models where presence_penalty is immutable). |
