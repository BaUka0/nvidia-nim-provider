...
describe("convertMessages", () => {
  ...
  it("strips failover notices from assistant messages in history", () => {
    const messages = [
      {
        role: vscode.LanguageModelChatMessageRole.Assistant,
        content: [
          new vscode.LanguageModelTextPart(
            "> ⚡ **NVIDIA NIM Fallback:** Timeout on *GLM 5.3 Flash*. Response generated by *Kimi K3*.\n\n" +
              "> ⚡ **NVIDIA NIM Fallback:** Timeout on *Kimi K3*. Response generated by *DeepSeek V4 Flash 0731*.\n\n" +
              "> ⚡ **NVIDIA NIM Fallback:** Timeout on *DeepSeek V4 Flash 0731*. Response generated by *Muse Glimmer*.\n\n" +
  ...
  it("handles assistant message consisting only of fallback notices", () => {
    const messages = [
      {
        role: vscode.LanguageModelChatMessageRole.Assistant,
        content: [
    ...
    const result = convertMessages(makeChatMessages(...messages));
    expect(result).toEqual<NimChatMessage[]>([{ role: "assistant", content: "(empty message)" }]);
  });
...
describe("stripFallbackNotices", () => {
  it("leaves clean text untouched", () => {
    const text = "This is a clean assistant response without any fallback notice.";
    expect(stripFallbackNotices(text)).toBe(text);
  });

  it("strips standard single fallback notice", () => {
    const text =
      "> ⚡ **NVIDIA NIM Fallback:** Timeout on *GLM 5.3 Flash*. Response generated by *Kimi K3*.\n\n" +
      "Here is the answer.";
    expect(stripFallbackNotices(text)).toBe("Here is the answer.");
  });

  it("strips multiple consecutive fallback notices", () => {
    const text =
      "> ⚡ **NVIDIA NIM Fallback:** Timeout on *GLM 5.3 Flash*. Response generated by *Kimi K3*.\n\n" +
      "> ⚡ **NVIDIA NIM Fallback:** Timeout on *Kimi K3*. Response generated by *DeepSeek V4 Flash 0731*.\n\n" +
      "> ⚡ **NVIDIA NIM Fallback:** Timeout on *DeepSeek V4 Flash 0731*. Response generated by *Muse Glimmer*.\n\n" +
      "Response text after three fallbacks.";
    expect(stripFallbackNotices(text)).toBe("Response text after three fallbacks.");
  });

  it("strips multiline wrapped blockquote notice", () => {
    const text =
      "> ⚡ **NVIDIA NIM Fallback:** Timeout on *GLM 5.3 Flash*.\n" +
      "> Response generated by *Kimi K3*.\n\n" +
      "Real content.";
    expect(stripFallbackNotices(text)).toBe("Real content.");
  });

  it("preserves standard user/assistant markdown blockquotes", () => {
    const text =
      "> Note: this is a user documentation quote that must stay intact.\n" +
      "> Follow this rule.\n\n" +
      "Understood!";
    expect(stripFallbackNotices(text)).toBe(text);
  });

  it("returns empty string when message contains only fallback notices", () => {
    const text =
      "> ⚡ **NVIDIA NIM Fallback:** Timeout on *GLM 5.3 Flash*. Response generated by *Kimi K3*.\n\n";
    expect(stripFallbackNotices(text)).toBe("");
  });
});
