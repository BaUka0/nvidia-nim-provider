...
- **Longer HTTP 503 retry pauses (`src/shared/constants.ts`, `src/shared/cancellation.ts`, `src/api/client.ts`, `src/provider/turn-executor.ts`, `src/provider/chat-provider.ts`).** `UNAVAILABLE_RETRY_MULTIPLIER` is 3. `exponentialRetryDelayMs` scales the in-stream transient pause (1s, 2s, 4s, cap 5s becomes 3s, 6s, 12s, cap 15s) and the failover-chain restart pause (first wait 2s becomes 6s, cap 10s becomes 30s) when `NvidiaApiError.status === 503`. `calculateRetryDelay` uses the same multiplier for an HTTP 503 (base 1s becomes 3s, cap 30s becomes 90s) and still honors `Retry-After`. 429, 502, 504, 529, and network delays stay on the short schedule.
- **Nemotron tool sampling (`src/models/adapters/nemotron.ts`, `src/provider/request-builder.ts`).** `toolTemperature` is `0.6` on Nemotron family and Nemotron adapters. `parallel_tool_calls` is sent only when an adapter or model option sets it, so a normal Nemotron tool turn omits the field.
- **Reasoning mode automap (`src/models/adapters/base.ts`, `deepseek.ts`, `glm.ts`, `nemotron-super.ts`, `nemotron-lightning.ts`, `src/provider/request-builder.ts`).** `resolveReasoningMode` maps an unsupported mode such as `on` or `auto` onto the closest supported mode. GLM `none` and `medium` resolve to `low`; `on` and `auto` resolve to `high`.
- **Read line span (`src/shared/constants.ts`).** `MAX_REPAIRED_LINE_SPAN` is `2000`, so an omitted read range fills `startLine = 1` and `endLine = start + 1999`.
- **Calmer nudges (`src/provider/loop-breaker.ts`, `src/tools/invalid-call-messages.ts`).** In-turn, history, and escalation breaker text no longer starts with "hey you got stuck". Invalid-tool retries no longer say "Retry NOW". Markers `[NIM_LOOP_BREAKER]` and `[NIM_LOOP_BREAKER_GO]` are unchanged.

### Fixed

- **Dropped streams continue instead of ending the turn (`src/api/client.ts`, `src/provider/stream-pump.ts`, `src/provider/attempt-retry.ts`, `src/provider/loop-breaker.ts`, `src/provider/attempt-loop.ts`).** `streamChatCompletion` now tracks the terminal SSE `[DONE]` sentinel and throws `StreamDroppedError` when the response body ends without it. The pump keeps the partial output and sets `streamDropped` when no `finish_reason` was seen either; a missing sentinel after a real `finish_reason` is tolerated and only logged (`missingDoneSentinel`). The evaluation gains the `stream_dropped` auto-continue reason with its own nudge, gated on reported text or reasoning, on no emitted tool call, and on the same-turn loop budget; a dropped stream with no output still goes through the empty-stream path. Addresses #15.
- **Section labels and quoted code lines (`src/provider/repetition-guard.ts`, `src/provider/stream-pump.ts`, `src/provider/attempt-loop.ts`, `src/shared/turn-report.ts`).** A normalized line of at most two words (`specific issues`, `recommendations`) only trips `lineCounter` when the copies are back to back. Blank lines do not break that run. A code-like line outside a fence (`{}`, `=>`, `?:`, `::`, or a trailing `;`) needs 8 consecutive copies, so `apiKey?: string;` quoted between sentences does not restart reasoning. Other lines still accumulate to `maxRepeatedLines`. Each trip log and the turn report include `detector`: `lineCounter`, `phrase`, `paragraph`, or `runaway`. A clean `finish_reason=stop` still retries when a real sentence loop trips, because tool turns release the held answer in one piece after the model has already stopped.
- **Reasoning repetition is looser than the visible answer (`src/provider/repetition-guard.ts`, `src/provider/stream-pump.ts`, `src/shared/cycle-detection.ts`).** The reasoning guard floors identical lines at 6 and asks `detectPhraseCycle` for a 12-word gram, so a normalized tool name such as `use the replace string in file` no longer stops the stream. `detectParagraphEcho` still trips when three paragraphs of at least 20 longer words share at least 14 of those words (Jaccard 0.62), which is the synonym-paragraph loop. Visible answers keep the 6-word gram and the configured line threshold. Runaway characters still trip in both guards. Addresses #7.
- **Tool-turn plans stay in thinking (`src/provider/stream-pump.ts`, `src/provider/tool-call-aggregator.ts`).** Embedded tool calls inside `reasoning_content` are executed and the tags are removed from the thinking part. After reasoning has started on a tool-enabled turn, later `content` is held. If that hold or the turn contains a tool call, the prose is folded into thinking instead of the chat. A turn with no tool call still releases the hold as the answer. A native call that repeats one already emitted from thinking is not executed twice.
- **Apostrophes in `isTokenInStringOrRegexLiteral` (`src/tools/xml-tool-scanner.ts`).** A letter-apostrophe-letter sequence is a contraction only for a closed English suffix (`s`, `t`, `ll`, `re`, `ve`, `d`, `m`). `return'hello <tool_call>'` stays a string. Plural possessives such as `users'` do not hide the following tool call.

### Removed

- **Adapter system prompts (`src/models/adapters/*`).** `toolSystemMessage` and `VISIBLE_REPLY_HYGIENE_MESSAGE` are gone. `getProfile` returns `extraSystemMessages: []`.
- **Sampling penalties (`package.json`, `src/types.ts`, `src/shared/config.ts`, `src/provider/request-builder.ts`, `src/shared/turn-report.ts`).** Removed `nvidia-nim.generation.frequencyPenalty` and `presencePenalty`, plus `frequency_penalty`, `presence_penalty`, and `repetition_penalty` from the request and turn report.
