...
- **JSON tool scanner isolation and non-tool preservation (`src/tools/json-tool-scanner.ts`, `src/provider/stream-pump.ts`, `tests/tools-parser.test.ts`).** In `scanJsonToolConstruct`, skipped `consumed` characters on non-tool JSON values to prevent re-scanning nested objects within non-tool wrappers, and required closing fences before committing fenced JSON. In `extractSingleToolCall` and `getIncompleteJsonToolCallName`, required explicit tool names to exist in `toolSchemas`, preventing disabled-tool JSON from being consumed as tool calls. Replaced loose property-string matching with `findBestMatchingTool` and flushed held pending embedded content as normal text or thinking at stream end.
