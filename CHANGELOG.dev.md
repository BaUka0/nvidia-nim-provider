...
- **Response detail extraction guards and mock response contracts (`src/api/client.ts`, `tests/helpers/fakes.ts`, `tests/api.test.ts`).** Added `typeof response.text === "function"` guard and `finally` block `clearTimeout(timerId)` cleanup in `readResponseDetail` to prevent timer leaks when detail parsing fails. Provided default `text` and `json` async handlers in `makeFetchResponse` harness. Replaced `.rejects.toMatchObject` assertion with `.catch(...)` in `tests/api.test.ts` to prevent Jest matcher recursion stack exhaustion (`RangeError`) on macOS arm64.
