...
- **System message turn consolidation (`src/provider/request-builder.ts`, `tests/provider/request-builder.test.ts`).** In `NimRequestBuilder.convertMessagesWithProfile`, extra adapter system guidance (`toolSystemMessage` and `VISIBLE_REPLY_HYGIENE_MESSAGE`) is now consolidated into a single string block. When the incoming request already begins with a system turn, adapter directives are merged directly into that turn rather than prepending multiple distinct `{ role: "system" }` messages, eliminating multi-system turn context pollution.
- **Softened duplicate read tool suppression with edit resets (`src/tools/canonical-key.ts`, `src/provider/tool-call-aggregator.ts`, `tests/tools-parser.test.ts`, `tests/provider/chat-provider.context.test.ts`, `tests/provider/chat-provider.tools.test.ts`).** Replaced binary duplicate suppression with `DEFAULT_MAX_DUPLICATE_READS = 2`, permitting legitimate re-reads and verification passes within a turn while cutting off runaway read loops on the 3rd consecutive identical call. Added automatic read count resets whenever an intervening edit tool (`isEditTool`) executes, preventing stale duplicate blocks after file modifications. Resolves #8, Resolves #12.
